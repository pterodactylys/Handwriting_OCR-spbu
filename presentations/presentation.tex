...
\begin{frame}
  \frametitle{Пример готового решения}
   \begin{center}
    \hspace*{-2.9cm}\includegraphics[width=1\textwidth,height=0.8\textheight,keepaspectratio]{assets/hwr_example_1.png}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \vfill
  \textbf{Анализ показал, что наиболее подходящим является метод CRNN + CTC.}
\end{frame}
...
\begin{frame}
  ...
  \begin{center}
    \hspace*{-0.55cm}\includegraphics[width=1.08\textwidth,height=1.05\textheight,keepaspectratio]{assets/segmentation_1.png}
  \end{center}
\end{frame}
...
\begin{frame}
  \frametitle{Препроцессинг изображений}
После разбиения на строки, изображения проходят следующие этапы препроцессинга:
\begin{itemize}
    \item \textbf{Перевод в градации серого} - упрощает данные и снижает влияние цветовых шумов.
    \item \textbf{Сжатие до фиксированного размера} - обеспечивает единообразие входных данных для модели.
    \item \textbf{Аугментация} - случайные искажения, повороты, добавление шума для увеличения разнообразия обучающей выборки и повышения устойчивости модели.
\end{itemize}
\end{frame}
